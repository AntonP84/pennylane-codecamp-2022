...
def average_fidelity(
    gate_list: list[str], wire_list: list[int], noise_param: float, over_rot: float
) -> float:
    ...
    Args:
        - gate_list: Gates, assumed to be non-parametric, contained in the ideal circuit.
        - wire_list: Wires on which each of the gates in gate_list act
        - noise_param: The noise parameter characterizing the depolarizing gate after the sqrt(iSWAP) gates
        - over_rot: Extra rotation angle on each rotation gate.
    Returns:
    ...
    def custom_cnot(params, wires):
        """Return custom CNOT gate as implemented by a superconducting qubit as an array, including noise"""
        beta, theta = params
        pi_2 = np.pi / 2

        return [
            qml.RZ(-pi_2 + theta, wires=wires[0]),
            qml.RX(pi_2 + theta, wires=wires[1]),
            qml.RZ(pi_2 + theta, wires=wires[1]),
            qml.ISWAP(wires=wires),
            qml.RX(pi_2 + theta, wires=wires[0]),
            qml.ISWAP(wires=wires),
            qml.RZ(pi_2 + theta, wires=wires[1]),
            qml.DepolarizingChannel(beta, wires=wires[0]),
            qml.DepolarizingChannel(beta, wires=wires[1]),
        ]

    custom_decomps = {
        qml.CNOT: functools.partial(custom_cnot, params=[noise_param, over_rot])
    }
    ...
    def fidelity(phi, theta):
        """Return the fidelity of the final state of the circuit,
        with respect to the ideal circuit,
        given an initial state parametrized by phi and theta"""
        return qml.math.fidelity(
            superconducting_circuit(phi, theta), reference_circuit(phi, theta)
        )

    fidelities = [fidelity(phi, theta) for theta, phi in sample]
...
def check(solution_output: str, expected_output: str) -> None:
    ...
    ), f"Your calculated average fidelity isn't quite right. Expected {expected_output:.2f}, got {solution_output:.2f}"


test_cases = [
    ['[["Hadamard","CNOT","PauliZ"],[0,[0,1],0],0.0001,0.01]', "0.363157"],
    ['[["CNOT","PauliZ","PauliZ"],[[1,0],1,0],0.0001,0.01]', "0.445426"],
]

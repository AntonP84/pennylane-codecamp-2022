...
def my_parameter_shift_grad(params: list[float], shift: float):
    ...
    for i in range(len(params)):
        params_plus, params_minus = params.copy(), params.copy()
        params_plus[i] += shift
        params_minus[i] -= shift

        gradient[i] = (circuit(params_plus) - circuit(params_minus)) / 2 / np.sin(shift)
